				...
				If e\EventState = 0 Then
					...
					;	SetNPCFrame(e\room\NPC[1], 210)
						...
						;	MoveEntity e\room\NPC[3]\Collider,1,0,0
							...
							;SetNPCFrame(e\room\NPC[4], 40) : e\room\NPC[4]\State = 3
							;RotateEntity e\room\NPC[4]\Collider,0,270,0
							;MoveEntity e\room\NPC[4]\Collider,5.5,1.0,-8.45
							...
							e\room\NPC[6] = CreateNPC(NPCtypeD, EntityX(e\room\Objects[8], True), 0.5, EntityZ(e\room\Objects[3], True))
							;PointEntity(e\room\NPC[6]\Collider, e\room\Objects[7])
							SetNPCFrame(e\room\NPC[6], 40) : e\room\NPC[6]\State = 3
							RotateEntity e\room\NPC[6]\Collider,0,90,0
							MoveEntity e\room\NPC[6]\Collider,5.5,1.0,-8.45
			...
			;;								BlinkTimer = -10
                      ...
                      ;                      Else
                       ;                         PositionEntity Curr173\Collider, 0,0,0
						...
						;					ResetEntity Curr173\Collider
											Msg = "Hold "+KeyName(KEY_SPRINT)+" to run, and most importartly find a gun."
											MsgTimer = 70*2
				...
				;		EndIf
					...
					If (e\EventState < 2000) Then
						If e\SoundCHN = 0 Then
							e\SoundCHN = PlaySound_Strict("SFX\Ending\GateB\EndingB3.ogg.ogg");)
						EndIf
					;	Else
				...
				;		If i = 0 Then PlaySound_Strict(LoadTempSound("SFX\Room\Intro\PA\scripted\scripted6.ogg"))
						...
						If (i>0 And i<26) Then
							If Not CommotionState(i) Then ;Prevents the same commotion file from playing more then once. Play franklin sound
								;PlaySound_Strict(LoadTempSound("SFX\Room\Intro\Commotion\Commotion"+i+".ogg"))
			...
			Case "173" ;the intro sequence
					...
					Else
						...
						If e\EventState = 0 Then
							If PlayerRoom = e\room Then
								...
								PositionEntity Collider, PlayerRoom\x-(14)*RoomScale, 0.3, PlayerRoom\z+192.0*RoomScale
								ResetEntity Collider
				...
				If PlayerRoom = e\room Then
					If e\EventState >= 10 Then
						CameraRange(Camera, 0.01, 15)
						If e\room\NPC[7]<>Null Then
							RemoveNPC(e\room\NPC[7])
						EndIf
					Else															
						CameraRange(Camera, 0.01, 40)
					EndIf	
									...
									If e\EventState2 < 2 Then
										If SelectedItem<>Null Then
											If SelectedItem\itemtemplate\tempname="25ct" Or SelectedItem\itemtemplate\tempname="coin" Then
												...
												inserted = True
											EndIf
											If SelectedItem\itemtemplate\tempname="misc" Or SelectedItem\itemtemplate\tempname="Mastercard" Then
												SelectedItem=Null
												e\EventState2 = e\EventState2 + 2
												PlaySound_Strict LoadTempSound("SFX\Interact\KeycardUse2.ogg")
												inserted = True
											EndIf
										EndIf
									EndIf
			...
			Case "room079"
				...
				If PlayerRoom = e\room Then
					...
					If e\EventState = 0 Then
						...
						e\room\NPC[1]=CreateNPC(NPCtype966, EntityX(e\room\Objects[5],True), EntityY(e\room\Objects[5],True)+0.5, EntityZ(e\room\Objects[5],True))
						;PointEntity e\room\NPC[0]\Collider, e\room\obj
						;RotateEntity e\room\NPC[0]\Collider, 0, EntityYaw(e\room\NPC[0]\Collider),0, True
					...
					ShouldPlay = 4
					If e\EventState < 10000
						If e\EventState = 1 Then 
							e\EventState = 2
						ElseIf e\EventState = 2
							;If EntityDistance(e\room\Objects[0], Collider)<0.1 Then 
							If (Distance(EntityX(Collider), EntityZ(Collider), EntityX(e\room\Objects[5],True), EntityZ(e\room\Objects[5],True))<4.5) Then
								e\EventState = 3
								e\EventState2 = 1
						...
						;ElseIf e\EventState = 3
							;e;lseIf e\EventState < 2000 Then ;3500
							;If ChannelPlaying(e\SoundCHN) Then
						...
						;	EndIf
						;Else
							;If EntityDistance(e\room\Objects[0], Collider)>0.5 Then 
								If EntityDistance(e\room\Objects[5], Collider)<0.1 Then 
								;EntityAlpha e\room\Objects[0],0.4
								;e\EventState = 10001
