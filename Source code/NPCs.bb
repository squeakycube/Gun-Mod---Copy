...
Dim KneelBlinkTimer%(2), KneelBlinkDuration%(2)
...
Function CreateNPC.NPCs(NPCtype%, x#, y#, z#)
	...
	Select NPCtype
		Case NPCtype173
			...
			ScaleEntity n\HitBoxHead%,MeshWidth(n\obj)/2.5,MeshHeight(n\obj)/8.0,MeshDepth(n\obj)/2.5
			PositionEntity n\HitBoxHead%,0,1.0,0,True
		...
		Case NPCtypeOldMan
			...
			n\HitBox% = CreateCube(n\obj)
			ScaleEntity n\HitBox%,MeshWidth(n\obj)/5.75,MeshHeight(n\obj)/2.5,MeshDepth(n\obj)/3.0
			PositionEntity n\HitBox,0,0.47,0.03,True
			EntityAlpha n\HitBox%,0.0
			EntityPickMode n\HitBox%,2
			
			n\HitBoxHead% = CreateCube(n\obj)
			ScaleEntity n\HitBoxHead%,MeshWidth(n\obj)/10.0,MeshHeight(n\obj)/5.0,MeshDepth(n\obj)/4.0
			PositionEntity n\HitBoxHead%,0,0.85,0.05,True
			EntityAlpha n\HitBoxHead%,0.0
			EntityPickMode n\HitBoxHead%,2
		...
		Case NPCtypeMTF
			...
			n\HitBoxHead% = CreateCube(n\obj)
			ScaleEntity n\HitBoxHead%,MeshWidth(n\obj)/9.0,MeshHeight(n\obj)/4.0,MeshDepth(n\obj)/3.0
			PositionEntity n\HitBoxHead%,0,-0.1,0.65,True
			EntityAlpha n\HitBoxHead%,0.0
			EntityPickMode n\HitBoxHead%,2
		...
		Case NPCtype049
			...
			n\HitBoxHead% = CreateCube(n\obj)
			ScaleEntity n\HitBoxHead%,MeshWidth(n\obj)/10.0,MeshHeight(n\obj)/5.0,MeshDepth(n\obj)/4.0
			PositionEntity n\HitBoxHead%,0,0.85,-0.03,True
			EntityAlpha n\HitBoxHead%,0.0
			EntityPickMode n\HitBoxHead%,2
		...
		Case NPCtypeTentacle
			...
			n\HitBox% = CreateCube(n\obj)
			ScaleEntity n\HitBox%,MeshWidth(n\obj)/3.0,MeshHeight(n\obj)/2.5,MeshDepth(n\obj)/3.0
			PositionEntity n\HitBox,0,0.0,0,True
		...
		Case NPCtype939
			...
			n\HitBox% = CreateCube(n\obj)
			ScaleEntity n\HitBox%,MeshWidth(n\obj)/3.0,MeshHeight(n\obj)/3.5,MeshDepth(n\obj)/1.0
			PositionEntity n\HitBox,0,0.0,0,True
			EntityAlpha n\HitBox%,0.0
			EntityPickMode n\HitBox%,2
		...
		Case NPCtype066
			...
			n\HitBox% = CreateCube(n\obj)
			ScaleEntity n\HitBox%,MeshWidth(n\obj)/2.5,MeshHeight(n\obj)/0.5,MeshDepth(n\obj)/0.5
			PositionEntity n\HitBox,0,0.0,0,True
			EntityAlpha n\HitBox%,0.0
			EntityPickMode n\HitBox%,2
		...
		Case NPCtype966
			;[Block]
			;[Block]
			n\NVName = "Chaos Insurgency Agent"
			n\Collider = CreatePivot()
			EntityRadius n\Collider, 0.32
			EntityType n\Collider, HIT_PLAYER
...
;			n\HitBox% = CreateCube(n\obj)
;			ScaleEntity n\HitBox%,MeshWidth(n\obj)/4.0,MeshHeight(n\obj)/4.5,MeshDepth(n\obj)/1.0
;			PositionEntity n\HitBox,0,0.1,0.65,True
;			EntityAlpha n\HitBox%,1.0
;			EntityPickMode n\HitBox%,2
;			
;			n\HP = 75
		...
		Case NPCtype008
			...
			MeshCullBox (n\obj, -MeshWidth(n\obj), -MeshHeight(n\obj), -MeshDepth(n\obj), MeshWidth(n\obj)*2.1, MeshHeight(n\obj)*2, MeshDepth(n\obj)*2.1)
			...
			n\HitBoxHead% = CreateCube(n\obj)
			ScaleEntity n\HitBoxHead%,MeshWidth(n\obj)/9.0,MeshHeight(n\obj)/4.0,MeshDepth(n\obj)/3.0
			PositionEntity n\HitBoxHead%,0,0.75,0,True
			EntityAlpha n\HitBoxHead%,0.0
			EntityPickMode n\HitBoxHead%,2
		...
		Select n\NPCtype
			Case NPCtype173
				...
				If Curr173\Idle <> 3 Then
					...
					Else ;idle = 2
						...
						If n\HP% < 1 Then
							AnimateNPC(n, 2, 26, 0.3, False)
							n\Idle = True
							n\IsDead = True
							GiveAchievement(AchvAntOpt)
						EndIf
				...
				EndIf
				If n\HP% < 1 Then
					AnimateNPC(n, 257, 108, 0.5, False)
					n\IsDead = True
				EndIf
				;[End block]
			Case NPCtypeOldMan ;------------------------------------------------------------------------------------------------------------------
				...
				Else
					...
					If (Not n\Idle) And spawn106%
							...
							Else
								...
								If Visible Then
									...
									If EntityInView(n\Collider, Camera) Then
										...
										;If dist > 4.8 Then
										If dist < 4.8 Then
											If Rand(270)=1 ;And ChannelPlaying(n\SoundChn2)=False Then n\PrevState = 0
                                    ;Injuries = Injuries+Rnd(0.4,1.0)
												Injuries = Injuries+Rnd(0.05,0.15)
										...
										;Melee
										If dist > 1.4 Then
											If Rand(100)=1 ;And ChannelPlaying(n\SoundChn2)=False Then n\PrevState = 0
												Injuries = Injuries+Rnd(0.2,0.5)
												AnimateNPC(n, 284, 333, n\CurrSpeed*43)
												DeathMSG = "Subject D-9341. Body partially decomposed by what is assumed to be SCP-106's "+Chr(34)+"corrosion"+Chr(34)+" effect. Body disposed of via incineration."
												If Not PlayerRoom\RoomTemplate\DisableDecals Then
													de.Decals = CreateDecal(0, EntityX(Collider), 0.01, EntityZ(Collider), 90, Rand(360), 0)
						...
						If n\HP < 1500 Then
							...
							If n\HP < 1500 Then
								If Rand(10)=1 Then
									KneelIng% = LoadImage_Strict("GFX\kneelmortal.pd")
									DrawImage (KneelIng%), GraphicWidth/2, GraphicHeight/2
							CameraShake = 3
						EndIf
					EndIf
				EndIf
			...
			Case NPCtype049
				...
				Else
					...
					If n\HP% > 1 Then
						If Rand(100)=1 Then
							n.NPCs = CreateNPC(NPCtypeZombie, EntityX(n\Collider), EntityY(n\Collider), EntityZ(n\Collider))
							n\State = 2
							EndIf
						EndIf
					EndIf
				...
				;	If n\HP% < 1 Then
				;		AnimateNPC(n, 637, 660, 0.7, False)
				;	EndIf
					
					If n\HP% < 1 Then
						AnimateNPC(n, 637, 660, 0.00001, True)
						GiveAchievement(AchvAntOpt)
						;C
						;Animate2(n\obj, AnimTime(n\obj), 637, 656, 0.7, False)
				EndIf
			...
			Case NPCtypeZombie
				...
				If Abs(EntityY(Collider)-EntityY(n\Collider))<4.0 Then
					...
					If (Not n\IsDead)
						...
						End Select
					Else
						AnimateNPC(n, 934, 935, 0.3, False)
					EndIf
			...
			Case NPCtypeGuard ;------------------------------------------------------------------------------------------------------------------
				...
				Select n\State
					Case 1 ;aims and shoots at the player
						...
						If KillTimer => 0 Then
							dist = EntityDistance(n\Collider,Collider)
							Local ShootAccuracy# = 0.1;+0.3*SelectedDifficulty\aggressiveNPCs
							...
							If PlayerRoom\RoomTemplate\Name = "exit1" Then
								...
								ShootAccuracy = ShootAccuracy + (0.2 - CurrSpeed*20)
							EndIf
	...
	AnimateNPC(n, 70, 162, 1.5, False)
			...
			Case NPCtype966
				...
				If (dist<HideDistance) Then
					...
					;RotateEntity n\obj,180.0,180.180,0
					
					PositionEntity n\obj,EntityX(n\Collider,True),EntityY(n\Collider,True)-0.3,EntityZ(n\Collider,True)
					RotateEntity n\obj,180.0,EntityYaw(n\Collider),180.0
...
;							Else
					AnimateNPC(n, 225.0, 225.0, 1.0, False)
;								
...
;							EndIf
							...
							n\Angle = VectorYaw(EntityX(Collider)-EntityX(n\Collider)+0.0,0,EntityZ(Collider)-EntityZ(n\Collider))
							RotateEntity n\Collider, 0, CurveAngle(n\Angle,EntityYaw(n\Collider),30.0),0
							
							;RotateEntity n\obj, 0, +180.0, 0
							;RotateEntity n\Collider, 0, +180, 0
						EndIf
                        ...
                        ;    n\State = 10
							...
							n\IsDead = True
							;Delay 500
							Msg = "You feel a sharp pain in your throat then next feel blood drip down your body."
							MsgTimer = 70*4
							Injuries = Injuries + Rnd(10.75,11.5)
							;Delay 1500 Then
							Kill()
							DeathMSG = "A bullet apeared to ricochet off of the jackass's hidden bullet proof vest killing both the Class-D and the spy"
						EndIf	
					;[End Block]
			Case NPCtype1048a
				;[Block]
	...
	Else
		Select Int(n\State) ;what is this MTF doing
			Case 0 ;wandering around
				...
				For n2.NPCs = Each NPCs
					...
					ElseIf n2\NPCtype = NPCtype008 And n2\IsDead = False
						If OtherNPCSeesMeNPC(n2,n) Then
							If EntityVisible(n\Collider,n\Collider)
								n\State = 9
								n\EnemyX = EntityX(n2\Collider,True)
								n\EnemyY = EntityY(n2\Collider,True)
								n\EnemyZ = EntityZ(n2\Collider,True)
			...
			Case 1 ;searching for player
                ...
                n\Speed = 0.015
                n\State2=n\State2-FPSfactor
                If (MeNPCSeesPlayer(n) = True) Or (n\HP% < 374) Then
			...
			If WearingVest>0 Then
				If WearingVest = 1 Then
					Select Rand(8)
						Case 1,2,3,4,5
							...
							Injuries = Injuries + Rnd(0.05,0.25)
						Case 6
							BlurTimer = 500
							ShotMessageUpdate = "A bullet hit your left leg."
							Injuries = Injuries + Rnd(0.1,0.15)
						Case 7
							BlurTimer = 500
				...
				Else
					If Rand(10)=1 Then
						...
						Injuries = Injuries + Rnd(0.3,0.45)
					Else
