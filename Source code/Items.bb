...
Function CreateItemTemplate.ItemTemplates(name$, tempname$, objpath$, invimgpath$, imgpath$, scale#, texturepath$ = "",invimgpath2$="",Anim%=0, texflags%=9, descr$="No Description.")
	...
	EndIf
	If temp = 7 Then
		descr = "A single sheet of paper that appears to be a document or note of some kind."
	EndIf
	If temp = 8 Then
		descr = "A large cigarette which appears to contain marijuana."
	EndIf
	If temp = 9 Then
		descr = "This thing looks pretty useless in this situation I don't think I should keep it."
	EndIf
	If temp = 10 Then
		descr = "A small styrofoam cup with liquid almost filling it up." ;A small styrofoam cup
	EndIf
	If temp = 11 Then
	...
	EndIf
	If temp = 18 Then
		descr = "A box containing various medical supplies that could help me if I was injured."
	EndIf
	If temp = 19 Then
		descr = "A small box containing a small amount medical supplies that could help me if I was slightly injured."
	EndIf
	If temp = 20 Then
	...
	EndIf
	If temp = 22 Then
		descr = "A small box with a noticeable blue tint containing a small amount medical supplies that could help me if I was slightly injured."
	EndIf
	If temp = 23 Then
		descr = "A hazmat suit designed to block out viruses, bacteria, prions, and other various related subjects. The suit has a noticeable lack of weight to it."
	EndIf
	If temp = 24 Then
		descr = "A hazmat suit designed to block out viruses, bacteria, prions, and other various related subjects. The suit has a noticeable weight to it."
	EndIf
	If temp = 25 Then
	...
	EndIf
	If temp = 27 Then
		descr = "Its an old navigation device used in mapping out cave systems, the foundation uses for new employees to find their way out of here."
	EndIf
	If temp = 28 Then
	...
	EndIf
	If temp = 64 Then
		descr = "An FN P90 likely created in Belgium, it is a high powered sub-machine gun for personal defence."
	EndIf
	If temp = 65 Then
		descr = "An AK-47, also called Kalashnikov Model 1947, is a high powered soviet assault rifle, possibly the most widely used shoulder weapon in the world."
	EndIf
...
Function InitItemTemplates()
	...
	CreateItemTemplate("FN P90", "p90", "GFX\items\P90_Worldmodel.b3d", "GFX\Guns\INV_icons\INVp90.jpg", "", 0.02)
	CreateItemTemplate("Crowbar", "crowbar", "GFX\items\Crowbar_Worldmodel.b3d", "GFX\items\INVcrowbar.jpg", "", 0.03)
	CreateItemTemplate("AK-47", "ak", "GFX\items\ak_w.b3d", "GFX\items\INVak.png", "", 0.02)
	CreateItemTemplate("m249", "m249", "GFX\Guns\m249\m249_worldmodel.b3d", "GFX\items\INVp90.jpg", "", 0.02)
	CreateItemTemplate("Pimp Gun", "pg", "GFX\Guns\m249\m249_worldmodel.b3d", "GFX\items\INVp90.jpg", "", 0.02)
	;----------------------------------------------------- AMMO --------------------------------------------------------------
	...
	it = CreateItemTemplate("Blood Bag", "blood", "GFX\items\blood bag.b3d", "GFX\items\INVBlood.png", "", 0.013)
	...
	it = CreateItemTemplate("Reminder!", "paper", "GFX\items\paper.x", "GFX\items\INVpaper.jpg", "GFX\items\docCode.png", 0.003) : it\sound = 0
