...
Global ErrorFile$ = "Errors\error_log_"
...
AppTitle "SCP - Gun Mod v"+VersionNumber
		...
		If KeyHit(28) And ConsoleInput <> "" Then
			...
			Select Lower(StrTemp)
				Case "help"
					...
					Select Lower(StrTemp)
						...
						Case "playmusic"
							...
							CreateConsoleMsg("******************************")
						Case "nightvision","nv"
							CreateConsoleMsg("HELP - nightvision")
							CreateConsoleMsg("******************************")
							CreateConsoleMsg("Turns on super nv")
							CreateConsoleMsg("******************************")
				...
				Case "nightvision", "nv"
					...
					;[End Block]
				Case "disablenightvision", "disablenv"
					;[Block]
			...
			If e\EventState = 1
				...
				If e\EventState2>30 Then
					...
					ElseIf e\EventStr = "load0"
						...
						;;e\room\Objects[2] = CreateCube(CreateCube, EntityX(e\room\Objects[2],True), EntityY(e\room\Objects[2],True), EntityZ(e\room\Objects[2],True))
						;e\room\Objects[2] = CreateCube(ObjectX(e\room\Objects[2],True), ObjectY(e\room\Objects[2],True), ObjectZ(e\room\Objects[2],True))
						QuickLoadPercent = 100
						e\EventState=2
						;CreateCube(ObjectX(e\room\Objects[2],True), ObjectY(e\room\Objects[2],True), ObjectZ(e\room\Objects[2],True)
					EndIf
				Else
...
Function MovePlayer()
	...
	Local Sprint# = 1.0, Speed# = 0.015, i%, angle#
...
Function DrawGUI()
				...
				Else
					If BlinkTimer < -3 And BlinkTimer > -10 Then
						If e\img = 0 Then
							If BlinkTimer > -5 Then
								If e\img = 0 Then
									e\img = LoadImage_Strict("GFX\kneelmortal.png")
	...
	If ClosestItem <> Null Then
		...
		AAText(GraphicWidth / 2 + Sin(yawvalue) * (GraphicWidth / 3) - 32,GraphicHeight / 2 - Sin(pitchvalue) * (GraphicHeight / 3) - 70, ClosestItem\name, True)
			...
			If Inventory(n) <> Null And SelectedItem <> Inventory(n) Then
				...
				If isMouseOn Then
					If SelectedItem = Null Then
								...
								If HoldingGun = 0 Then 
                                    ...
                                    If HoldingGun <> 0 Then
                                        ...
                                        If HoldingGun = 1
                                           ...
                                           ; Animate2(GUN_P90,AnimTime(GUN_P90),0,0,False)
                                        ElseIf HoldingGun = 2
                                            EqquipedGun\Deployed=0
                                            EqquipedGun\ShootState# = 0.0
                                            EqquipedGun\ReloadState# = 0.0
                                            EqquipedGun\ShootAnim = 0
                                           ; Animate2(GUN_USP,AnimTime(GUN_USP),0,0,False)
										ElseIf HoldingGun = 3
                                            EqquipedGun\Deployed=0
                                            EqquipedGun\ShootState# = 0.0
                                            EqquipedGun\ReloadState# = 0.0
                                            EqquipedGun\ShootAnim = 0
                                           ; Animate2(GUN_M9,AnimTime(GUN_M9),0,0,False)
										ElseIf HoldingGun = 4
                                            EqquipedGun\Deployed=0
                                            EqquipedGun\ShootState# = 0.0
                                            EqquipedGun\ReloadState# = 0.0
                                            EqquipedGun\ShootAnim = 0
                                           ; Animate2(GUN_M249,AnimTime(GUN_M249),0,0,False)
										ElseIf HoldingGun = 5
                                            EqquipedGun\Deployed=0
                                            EqquipedGun\ShootState# = 0.0
                                            EqquipedGun\ReloadState# = 0.0
                                            EqquipedGun\ShootAnim = 0
                                            ;Animate2(GUN_MP5,AnimTime(GUN_MP5),0,0,False)
											;PG
										ElseIf HoldingGun = 6
                                            EqquipedGun\Deployed=0
                                            EqquipedGun\ShootState# = 0.0
                                            EqquipedGun\ReloadState# = 0.0
                                            EqquipedGun\ShootAnim = 0
											RotateEntity BlaVlaBla, 190, 0, 0
                                        EndIf
                                    EndIf
                                Else 
                                    HoldingGun = 0
						...
						If HoldingGun = 6
							Msg = "You place your hand on your ballsack and start firing like a pimp, you feel unstoppable."
							MsgTimer = 70*4
							GodMode = True
							RotateEntity BlaVlaBla, + 90, -1, -1
						EndIf
                        ...
                        AASetFont Font1
                        Color 0,0,0
                        AAText(GraphicWidth / 2, GraphicHeight - 300 * MenuScale, Inventory(n)\itemtemplate\descr, True)                            
                        Color 255, 255, 255    
                        AAText(GraphicWidth / 2, GraphicHeight - 300 * MenuScale, Inventory(n)\itemtemplate\descr, True)  
					EndIf
				EndIf
			...
			Else
				...
				Else
					...
					ElseIf Inventory(MouseSlot) <> SelectedItem
						Select SelectedItem\itemtemplate\tempname
							Case "paper","misc","mastercard","key1","key2","key3","key4","key5","key6","misc","oldpaper","badge","ticket","25ct","coin","key","scp860,"
								;[Block]
		...
		If SelectedItem <> Null Then
			Select SelectedItem\itemtemplate\tempname
				...
				Case "battery"
					...
					;InvOpen = True
					;[End Block]
				Case "key1", "key2", "key3", "key4", "key5", "key6", "keyomni", "scp860", "hand", "hand2", "25ct", "misc","mastercard"
					;[Block]
				...
				Case "blood"
					...
					If Bloodloss = 0 And Injuries = 0 Then
						Msg = "You do not need to use a blood bag right now."
						MsgTimer = 70*5
						SelectedItem = Null
					Else
							...
							If SelectedItem\state = 100 Then
								...
								Else
									...
									If Injuries => 2.5 Then
										...
										Injuries = Max(2.5, Injuries-Rnd(2.3,2.7))
									ElseIf Injuries > 1.0
										Injuries = Max(1.5, Injuries-Rnd(2.5,3.0))
										If Injuries > 1.0 Then
				...
				Case "gift"
					...
					If CanUseItem(False,False,True)
						If Wearing714=1 Then
							Msg = Chr(34) + "Hey  if your seeing this you found a bug I never patched out -squeakycube" + Chr(34)
						Else
						...
						;	PlaySound_Strict LoadTempSound("SFX\Music\420J.ogg")
							For i = 1 To 1
								If Rand(3)=1 Then
									it.Items = CreateItem("FN P90","p90", EntityX(Collider,True)+Cos((360.0/20.0)*i)*Rnd(0.3,0.5), EntityY(Camera,True), EntityZ(Collider,True)+Sin((360.0/20.0)*i)*Rnd(0.3,0.5))
									;in mp replace with minigun
								Else
									;it.Items = CreateItem("Joint","420s", EntityX(Collider,True)+Cos((360.0/20.0)*i)*Rnd(0.3,0.5), EntityY(Camera,True), EntityZ(Collider,True)+Sin((360.0/20.0)*i)*Rnd(0.3,0.5))
									it.Items = CreateItem("First Aid Kit","firstaid", EntityX(Collider,True)+Cos((360.0/20.0)*i)*Rnd(0.3,0.5), EntityY(Camera,True), EntityZ(Collider,True)+Sin((360.0/20.0)*i)*Rnd(0.3,0.5))
									If Rand(3)=1 Then
										it.Items = CreateItem("SCP-500-01","scp500", EntityX(Collider,True)+Cos((360.0/20.0)*i)*Rnd(0.3,0.5), EntityY(Camera,True), EntityZ(Collider,True)+Sin((360.0/20.0)*i)*Rnd(0.3,0.5))
									Else
										it.Items = CreateItem("Document SCP-372","paper", EntityX(Collider,True)+Cos((360.0/20.0)*i)*Rnd(0.3,0.5), EntityY(Camera,True), EntityZ(Collider,True)+Sin((360.0/20.0)*i)*Rnd(0.3,0.5))
									...
									;ElseIf
										If Rand(3)=1 Then
											it.Items = CreateItem("Level 4 Key Card","key4", EntityX(Collider,True)+Cos((360.0/20.0)*i)*Rnd(0.3,0.5), EntityY(Camera,True), EntityZ(Collider,True)+Sin((360.0/20.0)*i)*Rnd(0.3,0.5))
										Else
											it.Items = CreateItem("Eyedrops","fineeyedrops", EntityX(Collider,True)+Cos((360.0/20.0)*i)*Rnd(0.3,0.5), EntityY(Camera,True), EntityZ(Collider,True)+Sin((360.0/20.0)*i)*Rnd(0.3,0.5))
										EndIf
								EndIf
							EndIf
								EntityType (it\collider, HIT_ITEM)
							Next
							PlaySound_Strict LoadTempSound("SFX\Interact\gift_open.ogg")
						EndIf
						MsgTimer = 70 * 5
						RemoveItem(SelectedItem)
					EndIf
				...
				Case "p90ammo"
					If P90Ammo% < 4
						P90Ammo% = P90Ammo% + 1
						;PlaySound("Guard Mod\GuardMod\SFX\guns\P90\pickup.ogg")
						;Msg = "You added 1 magazine for the FN P90."
						RemoveItem(SelectedItem)
					;	it = CreateItem("Empty 50 Rnd 5.7x28mm Ammo Magazine", "e57ammo", 1, 1, 1)
					Else
				...
				Case "uspammo"
					If uspAmmo% < 4
						uspAmmo% = uspAmmo% + 1
						;PlaySound("Guard Mod\GuardMod\SFX\guns\P90\pickup.ogg")
						;Msg = "You added 1 magazine for the H&K M4A1."
						RemoveItem(SelectedItem)
					;	it = CreateItem("Empty 50 Rnd 5.7x28mm Ammo Magazine", "e57ammo", 1, 1, 1)
					Else
		...
		ElseIf AchievementsMenu <= 0 And OptionsMenu > 0 And QuitMSG <= 0 And KillTimer >= 0
			...
			Select OptionsMenu
				...
				Case 4 ;Advanced
					...
					Color 255,255,255
					AAText(x, y, "Descriptions and advanced item names")
					ConsoleOpening = DrawTick(x + 270 * MenuScale, y + MenuScale, ConsoleOpening)
					If MouseOn(x+270*MenuScale,y+MenuScale,20*MenuScale,20*MenuScale)
						DrawOptionsTooltip(tx,ty,tw,th,"itemnames")
					EndIf
...
;	HideEntity OBJTunnel(6)
	...
	ParticleTextures(9) = LoadTexture_Strict("GFX\bloodsprite106.png", 1 + 2)
	...
	CameraRange Room2slCam, 0.01, 6.0
...
Function InitNewGame()
	...
	For r.Rooms = Each Rooms
		...
		If (r\RoomTemplate\Name = "start" And IntroEnabled = False) Then 
			PositionEntity (Collider, EntityX(r\obj)-1804*RoomScale, 704*RoomScale, EntityZ(r\obj)-524*RoomScale)
			PlayerRoom = r
