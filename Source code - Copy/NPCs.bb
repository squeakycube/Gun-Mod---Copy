...
Function CreateNPC.NPCs(NPCtype%, x#, y#, z#)
	...
	Select NPCtype
		...
		Case NPCtypeZombie
			...
			n\HitBox% = CreateCube(n\obj)
			ScaleEntity n\HitBox%,MeshWidth(n\obj)/5.0,MeshHeight(n\obj)/2.5,MeshDepth(n\obj)/3.0
			PositionEntity n\HitBox,0,0.08,0.5,True
			EntityAlpha n\HitBox%,0.0
			EntityPickMode n\HitBox%,2
			
			n\HitBoxHead% = CreateCube(n\obj)
			ScaleEntity n\HitBoxHead%,MeshWidth(n\obj)/10.0,MeshHeight(n\obj)/5.0,MeshDepth(n\obj)/4.0
			PositionEntity n\HitBoxHead%,0,0.075,0.74,True
			EntityAlpha n\HitBoxHead%,0.0
			EntityPickMode n\HitBoxHead%,2
							...
							Else
								...
								If Visible Then
									...
									If EntityInView(n\Collider, Camera) Then
										...
										;If dist > 4.8 Then
										If dist < 4.8 Then
											If Rand(200)=1 ;And ChannelPlaying(n\SoundChn2)=False Then n\PrevState = 0
                                    ;Injuries = Injuries+Rnd(0.4,1.0)
												Injuries = Injuries+Rnd(0.1,0.35)
...
Function UpdateMTFUnit(n.NPCs)
	...
	Else
		...
		If n\HP% < 1 Then
			AnimateNPC(n, 2, 26, 0.3, False)
			GiveAchievement(AchvkilledMTF)=True
		EndIf
...
;~F#123#134
;~B#1D5#1341#13DB#1474#1628#1733#18F4#1950
;~C#Blitz3D